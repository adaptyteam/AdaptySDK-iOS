...
extension Locale {
    ...
    func localized(numberOfPeriods discount: SKProductDiscount) -> String? {
        if discount.numberOfPeriods <= 1 { return nil }

        switch discount.subscriptionPeriod.unit {
        case .day:
            return localizedComponents(day: discount.numberOfPeriods)
        case .week:
            return localizedComponents(weekOfMonth: discount.numberOfPeriods)
        case .month:
            return localizedComponents(month: discount.numberOfPeriods)
        case .year:
            return localizedComponents(year: discount.numberOfPeriods)
        @unknown default:
            return nil
        }
    }
