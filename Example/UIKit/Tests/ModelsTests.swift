...
class ModelsTests: XCTestCase {
    ...
    func testFallbackPaywalls() throws {
        guard let url = bundle.url(forResource: "FallbackPaywalls", withExtension: "json") else {
            XCTFail("Missing file: FallbackPaywalls.json")
            return
        }
        let data = try Data(contentsOf: url)
        let json = try JSONSerialization.jsonObject(with: data, options: []) as! [String: Any]

        let paywallsArray = try FallbackPaywalls(json: json)
        expect(paywallsArray?.paywalls.count).to(equal(1))
        expect(paywallsArray?.products.count).to(equal(3))
    }
}
