...
class ViewController: UIViewController {
    ...
    func updateCustomerUserIdLabel() {
        customerUserIdLabel.text = "Customer User Id: \(Adapty.shared.customerUserId ?? "none")"
    }
    ...
    @IBAction func validateReceiptButtonAction(_ sender: Any) {
        ...
        setLoader(true)
        Adapty.shared.validateReceipt(receipt) { (response, error) in
            self.setLoader(false)
            if let error = error {
