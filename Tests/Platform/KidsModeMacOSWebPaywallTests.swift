...
@testable import Adapty_KidsMode
import Foundation
import Testing

struct PlatformKidsModeMacOSWebPaywallTests {
    ...
    func invalidCustomSchemeReturnsFalseOnNativeMacOS() {
        ...
        let result = invalidCustomSchemeURL.openmacOS(
            presentation: Adapty_KidsMode.AdaptyWebPresentation.externalBrowser,
    ...
    func invalidCustomSchemeWithInAppPresentationReturnsFalseOnNativeMacOS() {
        ...
        let result = invalidCustomSchemeURL.openmacOS(
            presentation: Adapty_KidsMode.AdaptyWebPresentation.inAppBrowser,
            canOpenExternalURL: { _ in false },
            openExternalURL: { _ in
                openWasCalled = true
                return true
